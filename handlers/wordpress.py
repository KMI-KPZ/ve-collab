...
class WordpressCollectionHandler(BaseHandler):
    ...
    def get(self):
        ...
        wp_response = requests.get("http://localhost/wp-json/wp/v2/posts/")
        wp_posts = json.loads(wp_response.content)
        
        self.write({"success": True,
                    "wordpress_posts": wp_posts})
...
class WordpressPostHandler(BaseHandler):
    ...
    def get(self, post_id):
        ...
        """
        wp_response = requests.get("http://localhost/wp-json/wp/v2/posts/{}".format(str(post_id)))
