...
export default function HeaderSection({
    ...
    const sandwichItemClass = 'block w-full px-2 py-1 whitespace-nowrap text-left hover:bg-slate-50'
                        ...
                        <Dropdown
                            ...
                            icon={
                                <div className='flex items-center'>
                                    <AuthenticatedImage
                                        imageId={userProfile ? userProfile.profile_pic : "default_profile_pic.jpg"}
                                        alt={'Benutzerbild'}
                                        width={30}
                                        height={30}
                                        className="rounded-full mr-3"
                                    ></AuthenticatedImage>
                                    <div title={`${userProfile?.first_name} ${userProfile?.last_name}`} className='max-w-[96px] truncate font-semibold'>
    ...
    const MenuMobile = () => {
        return (<>
            <li>
                <Link href="/learning-material" className={isActivePath('/learning-material') ? sandwichActiveItemClass : sandwichItemClass}>
                    Materialien
                </Link>
            </li>
            {session ? (<>
                <li>
                    <Link href="/groups" className={isActivePath('/group') ? sandwichActiveItemClass : sandwichItemClass}>Gruppen</Link>
                </li>
                <li>
                    <Link href="/plans" className={isActivePath(['/plans', '/ve-designer']) ? sandwichActiveItemClass : sandwichItemClass}>
                        <span className='text-ve-collab-orange'>VE</span> <span className='text-ve-collab-blue'>Designer</span>
                    </Link>
                </li>
                <li className={`relative`}>
                    <button className={sandwichItemClass} onClick={e => toggleChatWindow()}>
                        Chat
                    </button>
                    {messageEventCount > 0 && (
                ...
                <li className={`relative`}>
                    <button className={sandwichItemClass} onClick={e => toggleNotifWindow()}>
                        Benachrichtigungen
                    </button>
                    {notificationEvents.length > 0 && (
                ...
                <li>
                    <Link href={'/profile'} className={isActivePath(['/profile', '/profile/edit']) ? sandwichActiveItemClass : sandwichItemClass}>
                        Profil
                    </Link>
                </li>
                <li>
                    <button onClick={() => window.open('mailto:schlecht@infai.org, mihaela.markovic@uni-leipzig.de', '_blank')} className={sandwichItemClass}>Kontakt</button>
                </li>
                <li>
                    <button onClick={() => signOut()} className={sandwichItemClass}>Abmelden</button>
                </li>
            </>) : (<>
                <li>
                    <button onClick={() => signIn('keycloak')} className={sandwichItemClass}>Login</button>
                </li>
                <li>
                    <button onClick={() => signIn('keycloak')} className={sandwichItemClass}>Registrieren</button>
                </li>
                <li><hr className='mb-2 ' /></li>
                <li><Link href={'/search'} className={`px-2 py-1`}>Suche</Link></li>
            </>)}
        </>)
    }
