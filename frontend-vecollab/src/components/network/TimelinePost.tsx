...
export default function TimelinePost({
    ...
    const onSubmitCommentForm = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            const newComment = await fetchPOST(
                '/comment',
                { text, post_id: post._id },
                session?.accessToken
            );
    ...
    const pinComment = async (comment: BackendPostComment) => {
        try {
            if (comment.pinned) {
                await fetchDELETE(
                    '/pin',
                    { id: comment._id, pin_type: 'comment' },
                    session?.accessToken
                );
            } else {
                await fetchPOST(
    ...
    const Likes = () => {
        ...
        return (
            <div
                className="group/likes w-10 text-sm mr-3 my-4 flex relative hover:cursor-pointer overflow-hidden hover:overflow-visible"
                onMouseOver={() => fetchLikers(post.likers)}
            >
    ...
    const Comment = ({ comment }: { comment: BackendPostComment }) => (
        <>
            <div className={`flex items-center group/comment`}>
                ...
                <div
                    className={`ml-auto ${
                        !comment.pinned ? 'opacity-0' : ''
                    } transition-opacity group-hover/comment:opacity-100`}
                >
                    ...
                    <div className="inline opacity-0 group-hover/comment:opacity-100">
                        <CommentHeaderDropdown comment={comment} />
                    </div>
                </div>
            </div>
    ...
    const CommentHeaderDropdown = ({ comment }: { comment: BackendPostComment }) => {
        if (comment.author.username == session?.user.preferred_username || userIsAdmin) {
            return (
                <Dropdown
                    options={[
                        {
    ...
    return (
        <>
            ...
            <div
                ...
                className={`${
                    willRemvoe
                        ? 'opacity-0 transition-opacity ease-in-out delay-50 duration-300'
            ...
            >
                <div className="flex items-center">
                    ...
                    {!group && post.space && (
                        <div className="self-start leading-[1.6rem] text-xs text-gray-500 ml-1 text-nowrap">
                            <MdDoubleArrow className="inline" />{' '}
                            <Link href={`/group/${post.space}`} className="font-bold align-middle">
                                {GroupnameById(post.space)}
                            </Link>
                        </div>
                    )}
                ...
                {post.isRepost && (
                    <div className="my-5 ml-5 p-4 rounded bg-slate-100">
                        ...
                        <div
                            className={`${
                                repostExpand ? '' : 'max-h-40 overflow-hidden'
                            } mt-5 whitespace-break-spaces relative repost-text`}
                        >
                ...
                <div className="my-5">
                    {editPost ? (
                        ...
                        <TimelinePostText
                            text={
                                post.isRepost ? (post.repostText as string) : (post.text as string)
                            }
                        />
                    )}
                </div>
