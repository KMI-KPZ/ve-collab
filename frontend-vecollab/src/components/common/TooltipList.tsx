...
export const TooltipList = ({ children, tooltipsTextItems }: Props) => {
    return (
        <div className={`group relative inline-block`}>
            <div className="inline-flex rounded bg-primary cursor-help">{children}</div>
            <div
                className={`absolute bottom-full left-1/2 w-[20rem] z-20 mb-1 -translate-x-1/2 rounded bg-white border px-4 py-[6px] text-sm font-semibold hidden group-hover:block`}
            >
