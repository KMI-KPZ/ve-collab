...
export default function BroadStages() {
    ...
    const [steps, setSteps] = useState<BroadStep[]>([{ from: "", to: "", name: "" }, { from: "", to: "", name: "" }])
            ...
            .then((data) => {
                ...
                if (data.plan) {
                    if (data.plan.steps.length > 0) {
                        let list = data.plan.steps.map((step: any) => ({name: step.name, from: step.timestamp_from.split("T")[0], to: step.timestamp_to.split("T")[0]}))
                        setSteps(list)
                    }
                    else {
    ...
    const handleSubmit = async (e: FormEvent) => {
        steps.forEach(async step => {
            ...
            const response = await fetchPOST("/planner/append_step", { plan_id: planId, step: payload }, session?.accessToken)
            ...
            <div className="flex justify-between bg-pattern-left-blue-small bg-no-repeat">
                ...
                >
                    <div>
                        ...
                        <div className={"text-center mb-20"}>erstelle beliebig viele Etappen, setze deren Daten und vergib für jede einen individuellen Namen</div>
                        ...
                        <div className={"w-3/4 mx-7 mt-3 flex justify-end"}>
                            <button onClick={removeBox}><RxMinus size={20} /></button>
                            <button onClick={addBox}><RxPlus size={20} /></button>
                        </div>
                    </div>
