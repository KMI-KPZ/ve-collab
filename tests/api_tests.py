...
def setUpModule():
    ...
    global_vars.upload_directory = "unitest_uploads"
...
def tearDownModule():
    ...
    shutil.rmtree(global_vars.upload_directory, ignore_errors=True)
...
class PostHandlerTest(BaseApiTestCase):
    def setUp(self) -> None:
        ...
        if not os.path.isdir(
            os.path.join(global_vars.upload_directory, self.test_space)
        ):
            os.mkdir(os.path.join(global_vars.upload_directory, self.test_space))
    ...
    def tearDown(self) -> None:
        ...
        try:
            os.remove(os.path.join(global_vars.upload_directory, self.test_file_name))
    ...
    def test_post_create_post_space_with_file(self):
        ...
        self.assertIsNone(db_state["wordpress_post_id"])
        self.assertEqual(db_state["tags"], json.loads(request_json["tags"]))
        self.assertEqual(db_state["files"], [self.test_file_name])
        ...
        # assert file is stored on disk
        self.assertTrue(
            os.path.isfile(
                os.path.join(
                    global_vars.upload_directory, self.test_space, self.test_file_name
                )
            )
        )
    ...
    def test_delete_post_space_with_file(self):
        ...
        with open(
            os.path.join(global_vars.upload_directory, self.test_file_name), "wb"
        ) as fp:
        ...
        self.assertTrue(
            os.path.isfile(
                os.path.join(global_vars.upload_directory, self.test_file_name)
            )
        )
...
class SpaceHandlerTest(BaseApiTestCase):
    ...
    def test_get_space_files(self):
        ...
        self.db.spaces.update_one(
            ...
            {
                "$set": {
                    "files": [{"author": CURRENT_ADMIN.username, "filename": filename}]
                }
            },
        )
        ...
        self.assertTrue(
            any(file_obj["filename"] == filename for file_obj in response["files"])
        )
    ...
    def test_post_space_put_file(self):
        ...
        self.assertTrue(
            os.path.isfile(
                os.path.join(global_vars.upload_directory, self.test_space, file_name)
            )
        )
    ...
    def test_post_space_put_file_error_filename_collision(self):
        """
        expect: fail message because space already has a file with that name
        """

        file_name = "test_file.txt"

        # manually add file with same name to space
        self.db.spaces.update_one(
            {"name": self.test_space},
            {
                "$push": {
                    "files": {"author": CURRENT_ADMIN.username, "filename": file_name}
                }
            },
        )
        ...
        self.assertEqual(response["reason"], "filename_collision")
    ...
    def _setup_space_file(self, author: str):
        ...
        # ensure space directory inside uploads directory exists
        if not os.path.isdir(global_vars.upload_directory):
            os.mkdir(global_vars.upload_directory)
        if not os.path.isdir(
            os.path.join(global_vars.upload_directory, self.test_space)
        ...
        # save to disk
        with open(
            os.path.join(global_vars.upload_directory, self.test_space, filename), "wb"
        ) as fp:
            fp.write(b"this is a test text file")

        return filename

    def _assert_file_removed_from_space_and_filesystem(self, filename: str) -> None:
        """
        ...
        self.assertIn("files", db_state)
        self.assertFalse(
            any(file["filename"] == filename for file in db_state["files"])
        )
        self.assertFalse(
            os.path.isfile(
                os.path.join(global_vars.upload_directory, self.test_space, filename)
            )
        )
    ...
    def test_delete_space_delete_file_author(self):
        ...
        # manually add file
        filename = self._setup_space_file(CURRENT_USER.username)
        self.base_checks(
            "DELETE",
            "/spaceadministration/delete_file?name={}&file_name={}".format(
                self.test_space, filename
            ),
            True,
            200,
        )
        ...
        self._assert_file_removed_from_space_and_filesystem(filename)
    ...
    def test_delete_space_delete_file_error_no_file_name(self):
        ...
        self.assertEqual(response["reason"], MISSING_KEY_ERROR_SLUG + "file_name")
    ...
    def test_delete_space_delete_file_error_file_doesnt_exist(self):
        ...
        response = self.base_checks(
            "DELETE",
            "/spaceadministration/delete_file?name={}&file_name={}".format(
                self.test_space, "not_existing_file.txt"
            ),
            False,
            409,
        )
