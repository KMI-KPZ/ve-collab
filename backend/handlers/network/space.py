...
class SpaceHandler(BaseHandler):
    ...
    async def post(self, slug):
        ...
        if slug == "create":
            ...
            await self.invite_user_into_space(space_name, username)
            return
    ...
    async def invite_user_into_space(self, space_name: str, username: str) -> None:
        """
        ...
        with util.get_mongodb() as db:
            space_manager = Spaces(db)
            space = space_manager.get_space(
                space_name, projection={"members": True, "admins": True}
            )
